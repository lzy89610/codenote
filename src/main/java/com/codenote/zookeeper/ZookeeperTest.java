package com.codenote.zookeeper;
...
public class ZookeeperTest
{
	...
	private Watcher watcher = new Watcher()
	{
		...
		{
			System.out.println("testExistsWatch2  watch : {}" + event
					.getType());
		}
	};
...
//				"192.168.236.135:2181,localhost:2182,localhost:2183",
//				SESSION_TIMEOUT, watcher);
		
		zooKeeper = new ZooKeeper(
				"192.168.236.135:2181",
				SESSION_TIMEOUT, watcher);
	}
	...
	public void testCreate()
	{
		...
		try
		{
			result = zooKeeper.create("/zk001", "zk001data".getBytes(),
	...
	public void testGetDataWatch()
	{
		...
		try
		{
			byte[] bytes = zooKeeper.getData("/zk001", new Watcher()
			{
				public void process(WatchedEvent event)
				{
					System.out.println("testGetDataWatch  watch : {}" + event
									.getType());
				}
			}, null);
			result = new String(bytes);
		...
		try
		{
			zooKeeper.setData("/zk001", "lizeyutest2".getBytes(), -1);
		}
		catch (Exception e)
		{
			LOGGER.error(e.getMessage());
	...
	public void testSetData()
	{
		...
		try
		{
			stat = zooKeeper.setData("/zk001", "testSetData2".getBytes(), -1);
		}
		catch (Exception e)
		{
			LOGGER.error(e.getMessage());
	...
	public void testExistsWatch2()
	{
		...
		try
		{
			stat = zooKeeper.exists("/zk002", new Watcher()
			{
				...
				{
					LOGGER
							.info("testExistsWatch2  watch : {}", event
									.getType());
					System.out.println("testExistsWatch2  watch : {}" + event
									.getType());
				}
	...
	public void testGetChild()
	{
		try
		{
			zooKeeper.create("/zk/001", "001".getBytes(), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
			zooKeeper.create("/zk/002", "002".getBytes(), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
